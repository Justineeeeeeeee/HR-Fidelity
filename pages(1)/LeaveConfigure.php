          ...
          <div class="col-12" >
            <div class="card">
              ...
              <div class="card-body">
                ...
                $sql = "SELECT tbl_employees.managed_id, employee_leave.Leave_ID, employee_leave.Employee_Name,employee_leave.Leave_Date, employee_leave.Leave_Status,employee_leave.Leave_Type FROM `employee_leave` INNER JOIN tbl_employees ON tbl_employees.employee_name = employee_leave.Employee_Name WHERE tbl_employees.managed_id =  '$employee_id'";
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result)) { ?>
                      <tr id = <?php echo $row["Leave_ID"]; ?>>
                        <td><?php echo $i; ?></td>
                        <td  data-name = "<?php echo $row["Leave_ID"] ?>" ><?php echo $row["Leave_ID"]; ?></td>
                        ...
                        <td data-Password = "<?php echo $row["Leave_Type"]  ?>"  ><?php echo $row["Leave_Type"]; ?></td>
                        <td>
                          <?php
                            if($row["Leave_Status"])
                            {
                        ...
                        <td>
                          <?php
                            if($row["Leave_Status"])
                            { ?>
                              <button class="btn btn-primary btn-sm btn-activate" style = "width: 100px;"><i class="fas fa-user-check"></i>Approved</button>
                            <?php
                            }
                            else
                            { ?>
                              <button class="btn btn-primary btn-sm btn-activate" style = "width: 100px;"><i class="fas fa-user-check"></i>Approve</button>
                        <?php
                           }
                          ?>     
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    <label>Name</label>
                    <input type="text" class="form-control" name="Edit_Employee_Name" id="Edit_Employee_Name" placeholder="Name">
                  </div>
                  <div class="form-group">
                    <label>Username</label>
                    <input type="text" class="form-control" name="Edit_Leave_Date"  id="Edit_Leave_Date" placeholder="Username">
                  </div>
...
var Employee_name = $(this).closest("tr").children(":eq(2)").text();
...
{$.ajax({
      type: "POST",
      cache: false,
      url: 'Process/Process_Leave_Configure.php',
      data: {type:'activate',Edit_Leave_ID:Edit_Leave_ID, Employee_name:Employee_name},
      success:function(data){
        ...
        if(response_data.title == 'Success')
        {
          ...
          $actions.html($actions.html().toString().replace('btn-activate','btn-activate').replace('fas fa-user-check', 'fas fa-user-check').replace('Activate', 'Deactivate'));
          Swal.fire(
            'Approved',
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    $.ajax({
          ...
          cache: false,
          url: 'Process/Process_Leave_Configure.php',
          data: {type:'delete',Edit_Leave_ID:Edit_Leave_ID, Employee_name : Employee_name},
          success:function(data){
